...
const PickupDetail = () => {
  ...
  const { pickup, isLoading, isCompleting, error, completePickup } = usePickupDetail(id);

  const handleCompletePickup = useCallback(async () => {
    const success = await completePickup();
    if (success) {
      navigate('/pickups');
    }
  }, [completePickup, navigate]);

  const statusBadge = useMemo(() => {
    if (!pickup) return null;
    if (pickup.isCompleted) return { variant: 'success' as const, label: 'Completed' };
            ...
            <Card>
              ...
              <div className="text-center py-8">
                <div className="inline-block p-6 bg-white border-2 border-gray-100 rounded-xl shadow-sm">
                  <QrCode className="h-40 w-40 text-primary-600 mx-auto" />
                </div>
                <p className="text-sm text-gray-600 mt-4">
                  Code: <span className="font-mono font-bold text-gray-900">{pickup.qrCode}</span>
                </p>
