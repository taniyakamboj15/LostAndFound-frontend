...
import { Bell, Check, CircleAlert, Package, HandCoins, Info, ArrowUpRight } from 'lucide-react';
...
const getNotificationIcon = (event: string) => {
  switch (event) {
    case 'MATCH_FOUND': return <Package className="h-5 w-5 text-indigo-500" />;
    case 'PAYMENT_REQUIRED': 
    case 'PAYMENT_RECEIVED': return <HandCoins className="h-5 w-5 text-emerald-500" />;
    case 'CLAIM_STATUS_UPDATE': return <CircleAlert className="h-5 w-5 text-amber-500" />;
...
const getNotificationLink = (notification: Notification): string => {
  ...
  if(referenceId) return `/claims/${referenceId}`; // Fallback if referenceId = claimId
              ...
              <div className="divide-y divide-slate-100">
                {notifications.map((notification) => (
                  <button
                    ...
                    className={`w-full text-left p-4 hover:bg-slate-50 transition-colors flex gap-3 ${!notification.isRead ? 'bg-indigo-50/30' : ''}`}
                  >
